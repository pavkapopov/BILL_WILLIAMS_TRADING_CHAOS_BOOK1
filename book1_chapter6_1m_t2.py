...
trade_quantity = 0.03000000 # количество базовой валюты для покупки. должна быть больше по стоимости чем MIN_NOTIONAL т.е. trade_quantity * last_price > MIN_NOTIONAL
...
clist = requests.get("https://api.binance.com/api/v3/klines?symbol=XVSUSDT&interval=5m&limit=30").json()
close = []
for i in range(len(clist)):
    close.append(float(clist[i][4]))
sma5 = talib.SMA(np.array(close),5)
sma10 = talib.SMA(np.array(close),10)
...
def on_message(ws, message):
    ...
    global sma5_more_sma10
    global sma10_more_sma5
    global trade_symbol
...
#mfi
    ...
    if buy_long == 1 and squat_bar_long == 1 and sma10[-1] > sma5[-1]:
    ...
    if sma5[-1] > sma10[-1] and sma10_more_sma5 == 1:
        sma5_more_sma10 = 1
        sma10_more_sma5 = 0

    if sma10[-1] > sma5[-1] and buy_long == 0 and sma5_more_sma10 == 1:
...
# если тренд не получился выставляем отложенный ордер на продажу по чуть завышенной цене.        
            if last_price <= limit_sell_price and buy_long == 0  and sma5_more_sma10 == 1:
                timestamp = requests.get("https://api.binance.com/api/v3/time").json()
