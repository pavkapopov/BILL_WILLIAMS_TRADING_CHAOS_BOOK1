...
trade_quantity = 0.04 # количество базовой валюты для покупки. должна быть больше по стоимости чем MIN_NOTIONAL т.е. trade_quantity * last_price > MIN_NOTIONAL
...
if float(price["price"]) * float(trade_quantity) < float(dict_exchangeInfo["minNotional"]):
    print("Количество базовой валюты не достаточно для торгов, необходимо минимум", float(dict_exchangeInfo["minNotional"]) / float(price["price"]))
    exit()

print("Параметры валютной пары получены, можно торговать...")
...
sma3 = talib.SMA(np.array(close),3)
sma7 = talib.SMA(np.array(close),7)
...
#mfi
    ...
    if sma3[-1] > sma7[-1] and buy_long == 1 and squat_bar_long > 0 and squat_bar_long <= 5:
    ...
    if sma7[-1] > sma3[-1] and buy_long == 0:
...
#получаем данные по ордеру
        ...
        if order_status == "FILLED":
            ...
            limit_sell_price =  price_buy_long + price_buy_long * (0.001*2) + price_buy_long * 0.002
