...
#общие настройки
API_KEY = 'zLfgPxZsqFtjnfCeRDr2T9MjNM8XKiw9iNwGshfr75SEXZp03AMgVvVyz4b0V1do'
API_SECRET = 'sC3aTw7EGXdkXnlWUUejy98zwI5KJtRmUfd68nCZRkiQZpfJK2PCP2NDFyYhzUNz'
...
clist = requests.get("https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval=1m&limit=30").json()
close = []
for i in range(len(clist)):
    close.append(float(clist[i][4]))
ema5 = talib.EMA(np.array(close),5)
ema10 = talib.EMA(np.array(close),10)
...
#mfi
    ...
    if ema5[-1] > ema10[-1] and buy_long == 1 and squat_bar_long > 0 and squat_bar_long <= 5:
    ...
    if ema10[-1] > ema5[-1] and buy_long == 0:
